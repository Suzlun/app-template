...
export interface AdminI18n {
  ...
  /** 日時値を Admin 表示 locale に合わせて整形します。 */
  readonly formatDateTime: (
    value: Date | number | string,
    options?: Intl.DateTimeFormatOptions
  ) => string;
  /** 数値を Admin 表示 locale に合わせて整形します。 */
...
export function createAdminI18n(localeCandidate: string | null | undefined = null): AdminI18n {
  ...
  return {
    ...
    formatDateTime: (value, options = {}) => {
      // 日時表示は route ごとの `toISOString()` ではなく、shared i18n formatter に集約する。
      return runtime.formatters.dateTime(value, {
        dateStyle: 'medium',
        timeStyle: 'short',
        ...options,
      });
    ...
    formatNumber: (value, options = {}) => {
      // 件数表示などの数値も locale 固有の桁区切りに揃える。
      return runtime.formatters.number(value, options);
    },
    t: (key, values = {}) => {
...
/**
 * Admin Console 用 locale 候補を対応済み locale へ正規化します。
 *
 * @param candidate UI、browser、storage から取得した locale 候補です。
 * @returns `ja` または `en` に正規化できた場合はその値、対応外の場合は `null`。
 */
export function normalizeAdminLocale(candidate: string | null | undefined): AdminLocale | null {
  // shared i18n runtime の正規化規則だけを使い、Admin 独自の分岐を増やさない。
  const locale = normalizeLocale(candidate);
  if (locale === 'ja' || locale === 'en') return locale;

  // 対応外 locale は保存せず、呼び出し側が fallback を選べるよう null にする。
  return null;
}
