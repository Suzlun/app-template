...
const removedProductSdkImport = `from '${['@app-template', 'api'].join('/')}'`;
...
describe('Admin static client cleanup', () => {
  it('[ADMIN-CONSOLE-FE-S038] app layer cannot import Admin API client directly', () => {
    ...
    expect(eslintConfig).toContain('@app-template/web-admin-api');
    expect(accountsPage).toContain("from '@app-template/web-admin-domain'");
    expect(loginPage).toContain("from '@app-template/web-admin-domain'");
    expect(accountsPage).not.toContain("from '@app-template/web-admin-api'");
    expect(loginPage).not.toContain("from '@app-template/web-admin-api'");
  });

  it('[ADMIN-CONSOLE-FE-S039] Admin package rejects server-only modules', () => {
  ...
  it('SvelteKit config is static and lint config rejects server runtime reintroduction', () => {
    ...
    expect(eslintConfig).toContain('packages/web/admin/app/src/routes/**/+server.ts');
    expect(eslintConfig).toContain('packages/web/admin/app/src/lib/server/**/*.{ts,js,svelte}');
  });
  ...
  it('[ADMIN-AUTH-FE-S027/S028] login UI uses Admin auth domain and not Product SDK', () => {
    ...
    expect(authDomain).not.toContain(removedProductSdkImport);
    expect(eslintConfig).toContain('削除済み Product SDK alias を import しないでください');
  });

  it('[ADMIN-AUTH-FE-S012/S013/S014/S015/S016] Admin passkey management remains on admin surface', () => {
    // Admin passkey 一覧・追加・最終 passkey 削除抑止・複数件削除確認の UI contract を移動後 path で固定する。
    const passkeysPage = readPackageFile('src/routes/passkeys/+page.svelte');
    const accountPasskeyList = readPackageFile('src/lib/components/accounts/PasskeyList.svelte');

    expect(passkeysPage).toContain("i18n.t('passkeys.registeredTitle')");
    expect(passkeysPage).toContain('function handleAddPasskey(): void');
    expect(passkeysPage).toContain('disabled={passkeys.length <= 1 || deletingId === passkey.id}');
    expect(passkeysPage).toContain('function requestDelete(passkeyId: string): void');
    expect(passkeysPage).toContain('onConfirm={confirmDelete}');
    expect(accountPasskeyList).toContain('{#each passkeys as pk (pk.id)}');
    expect(accountPasskeyList).toContain('{#if onDelete != null && passkeys.length > 1}');
  });
