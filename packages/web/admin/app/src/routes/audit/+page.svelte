<script lang="ts">
	import { CardNS, EmptyState } from '@app-template/web-ui/components';
	...
	interface AuditData { events: AuditRow[]; operators: OperatorRow[]; page: number; totalPages: number; filters: Record<string, string> }

	const { data } = $props<{ data?: Partial<AuditData> }>();
	const pageData = $derived<AuditData>({
		events: data?.events ?? [],
	...
	const events = $derived(pageData.events.map((event: AuditRow) => ({ id: event.id, operator_email: pageData.operators.find((operator: OperatorRow) => operator.id === event.operatorId)?.email, action: event.action, target_type: event.targetType, target_id: event.targetId, outcome: event.outcome, outcome_label: event.outcome, details: event.details ?? undefined, created_at: i18n.formatDateTime(event.createdAt) })));
