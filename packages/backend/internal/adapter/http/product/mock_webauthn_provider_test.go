...
func (m *mockWebAuthnProvider) BeginRegistration(_ context.Context, accountID domain.AccountID) (string, []byte, error) {
	key := fmt.Sprintf("mock-reg-challenge-%d", m.seq.Add(1))
	m.mu.Lock()
	m.session[key] = accountID.String()
	m.mu.Unlock()
	...
	optionsJSON := `{"publicKey":{"rp":{"id":"localhost","name":"Test RP"},"user":{"id":"dXNlcmlk","name":"testuser","displayName":"Test User"},"challenge":"` + key + `","pubKeyCredParams":[{"type":"public-key","alg":-7},{"type":"public-key","alg":-257}],"authenticatorSelection":{"requireResidentKey":true,"residentKey":"required","userVerification":"required"}}}`
	return key, []byte(optionsJSON), nil
}

func (m *mockWebAuthnProvider) FinishRegistration(_ context.Context, _ string, _ domain.AccountID, credential application.WebAuthnAttestationCredentialDTO) (string, domain.WebAuthnCredentialData, error) {
	handle := credential.ID
	if handle == "" {
