...
func TestAdminValkeyNamespaceScenarios(t *testing.T) {
	t.Parallel()

	t.Run("[ADMIN-AUTH-BE-S062] Admin Valkey uses a separate logical DB on shared infrastructure", func(t *testing.T) {
	...
	t.Run("[ADMIN-AUTH-BE-S063] Admin Valkey keys use admin prefix and reject Product namespace drift", func(t *testing.T) {
		// Step 1: admin-prefixed key namespace の追跡点として、誤って Product 用 prefix が渡されても Admin store は key namespace を `admin:*` に固定する。
		...
		// Step 2: admin-prefixed keys only の挙動として、session key が `admin:*` で始まり、Product prefix や環境 prefix を先頭へ混入しないことを検証する。
