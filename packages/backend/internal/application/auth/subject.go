...
// OperatorSubjectID は Operator subject の application 境界 ID である。
//
// 役割:
//   - canonical auth lifecycle の公開 API で domain.OperatorID を直接露出させず、Operator subject payload の所有者 ID として表現する。
//   - 実体は domain.OperatorID と同じ値であり、OperatorAuth use case へ追加変換なしに渡せる。
type OperatorSubjectID = domain.OperatorID

// OperatorSubjectSessionID は OperatorAuth subject の application 境界 session ID である。
//
// 役割:
//   - Operator subject payload から session selector を返す際の公開 DTO 名を提供する。
//   - 実体は domain.OperatorSessionID と同じ値であり、refresh/session store へ安全に渡せる。
...
//   - Operator subject と共通の discriminator field を持たず、呼び出し元の service artifact 境界で意味を決める。
...
func NewAccountSubjectPayload(accountID domain.AccountID, sessionID string) (AccountSubjectPayload, error) {
	...
	// Step 2: session selector を Product AccountAuth 専用型として検証し、Operator session ID と型レベルで分ける。
...
// OperatorSubjectPayload は hosted service adapter が canonical auth lifecycle へ渡す Operator subject を表す。
//
// 役割:
//   - Operator の owner ID と operator session ID を明示的な subject payload として束ねる。
//   - Product Account subject と discriminator で切り替えず、service adapter がこの型を選んだ事実を境界にする。
//   - RBAC や active 判定は domain.Operator / OperatorAuthSession に残し、この payload は owner/session の対応だけを表す。
//
// 引数:
//   - NewOperatorSubjectPayload の operatorID: Operator の canonical ULID 文字列。
//   - NewOperatorSubjectPayload の sessionID: Operator session の canonical ULID 文字列。
//
// 戻り値:
//   - OperatorSubjectPayload: 検証済み Operator subject payload。
//   - error: operatorID または sessionID が operator/session として不正な場合の domain error。
//
// 使用例:
//
...
func NewOperatorSubjectPayload(operatorID string, sessionID string) (OperatorSubjectPayload, error) {
	// Step 1: OperatorID を Operator 専用 constructor で検証し、AccountID として扱わない。
	validatedOperatorID, err := domain.NewOperatorID(operatorID)
	if err != nil {
		return OperatorSubjectPayload{}, err
	}

	// Step 2: session selector を Operator session 専用型として検証し、Account session と混同しない。
...
// OperatorID は Operator subject の canonical owner ID を返す。
...
// SessionID は OperatorAuth subject の session selector を返す。
