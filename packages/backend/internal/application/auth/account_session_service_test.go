...
func TestAuthBES060IssueAccountSessionReturnsAccessTokenBodyAndRefreshTokenCookie(t *testing.T) {
	t.Parallel()
	...
	// Step 2: 外側 flow が検証済み AccountID を渡した状態を再現し、Product session issuance use case を実行する。
	result, err := service.IssueAccountSession(ctx, IssueAccountSessionInput{AccountID: accountAuth.AccountID(), ClientIP: "192.0.2.10", UserAgent: "test-agent"})
	if err != nil {
...
func assertProductLoginSessionBody(t *testing.T, result AccountSessionResult, accountAuth domain.AccountAuth) {
	t.Helper()
...
func assertProductLoginBodyHidesRefresh(t *testing.T, result AccountSessionResult) {
	t.Helper()
...
func assertProductLoginRefreshState(t *testing.T, result AccountSessionResult, refreshStore *testRefreshStore, accountAuth domain.AccountAuth, now time.Time) {
	t.Helper()
...
func assertProductLoginMetadata(t *testing.T, result AccountSessionResult, sessionStore *testSessionStore, accountAuth domain.AccountAuth) {
	t.Helper()
...
func TestRefreshAccountSessionConsumesCredentialWhenMetadataRevoked(t *testing.T) {
	t.Parallel()
	...
	// Step 2: metadata が存在しない session の refresh を実行し、提示済み refresh credential を再利用不能にするため Rotate へ到達することを確認する。
	...
	if !errors.Is(err, ErrAccountAuthUnauthorized) {
		t.Fatalf("expected ErrAccountAuthUnauthorized, got %v", err)
	}
	if !refreshStore.rotateCalled {
		t.Fatalf("refresh rotation must consume presented credential after session metadata has been revoked")
	}
}
...
func (s *testRefreshStore) Rotate(_ context.Context, tokenHash domain.OpaqueTokenHash, _ time.Duration, build RefreshRotationBuilder) (domain.AccountRefreshSession, domain.AccountRefreshSession, error) {
	// Step 1: Rotate 呼び出し有無を記録し、metadata 失効時にも提示 credential を消費する挙動を検証可能にする。
	s.rotateCalled = true
	if s.sessionsByTokenHash != nil {
...
func (s *testSessionStore) List(_ context.Context, accountID domain.AccountID) ([]SessionMetadata, error) {
	// Step 1: accountID に紐づく Product session metadata だけを抽出し、一覧 use case の canonical store port を満たす。
...
func mustTestService(t *testing.T, accountAuth domain.AccountAuth, refreshStore *testRefreshStore, sessionStore *testSessionStore, now time.Time) *AccountSessionService {
	t.Helper()

	// Step 1: test 用 signer と deterministic generator を注入した Product auth service を生成する。
	signer, err := NewTokenJSONSignVerifier([]byte("product-auth-test-secret"))
	if err != nil {
		t.Fatalf("create signer: %v", err)
	}
	service, err := NewAccountSessionService(AccountSessionDependencies{
		...
		Signer:          signer,
		IDGenerator:     &testIDGenerator{ids: []string{"01ARZ3NDEKTSV4RRFFQ69G5FB2", "01ARZ3NDEKTSV4RRFFQ69G5FB3", "01ARZ3NDEKTSV4RRFFQ69G5FB4", "01ARZ3NDEKTSV4RRFFQ69G5FB5", "01ARZ3NDEKTSV4RRFFQ69G5FB6"}},
		TokenGenerator:  &testTokenGenerator{tokens: []string{"new-refresh-token", "second-refresh-token"}},
		Clock:           func() time.Time { return now },
	}, AccountSessionConfig{AccessTokenTTL: 15 * time.Minute, RefreshTokenTTL: time.Hour, RefreshCookieLifetime: 30 * time.Minute})
	if err != nil {
...
func mustTestAccessToken(t *testing.T, signer JSONSignVerifier, accountID domain.AccountID, sessionID domain.AccountAuthSessionID, jtiValue string, now time.Time, ttl time.Duration) string {
	t.Helper()
