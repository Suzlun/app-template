...
| ツール             | pnpm 11.5.1, ESLint 9, Prettier, golangci-lint 1.64, Husky |
| CI                 | GitHub Actions                                             |
| 開発環境           | Zed + Dev Container（Docker Compose）                      |
...
### オプション A: Zed + Dev Container（推奨）

Zed で repository root を開き、Dev Container 起動の prompt から `Open in Container` を選びます。
...
Zed 接続後の terminal、task、language server は Dev Container 内で動作します。起動すると以下のサービスが自動で立ち上がります。
...
Zed の project task から `pnpm dev:*`、`pnpm lint`、`pnpm check`、`pnpm test:run`、`pnpm build` を実行できます。すべて Dev Container 内の `pnpm` と toolchain を使います。

Codex Desktop や host 側 terminal から検証する場合は、host の Node.js / Go / bash 差分を避けるため `scripts/devcontainer/run.sh` 経由で実行します。

```bash
scripts/devcontainer/run.sh pnpm check
scripts/devcontainer/run.sh pnpm lint
scripts/devcontainer/run.sh pnpm test:run
scripts/devcontainer/run.sh pnpm build
```
