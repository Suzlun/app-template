...
`compose.yaml` がローカル infra/runtime の唯一の定義です。setup 後はまず `devenv shell -- pnpm infra:up` を実行し、PostgreSQL / Valkey / OpenSearch / MinIO / Mailpit / SigNoz を起動してください。host / `devenv shell` で `pnpm dev:*` を動かす標準導線では optional な `runtime` container は起動しません。`runtime` container が必要な場合だけ、別途 `devenv shell -- pnpm runtime:up` を実行します。
...
| SigNoz OTLP              | gRPC: `127.0.0.1:4317` / HTTP: `127.0.0.1:4318`                 | gRPC: `signoz-otel-collector:4317` |

host / `devenv shell` で起動する Product/Admin backend は `.config/local.toml` と `.config/local.admin.toml` を使い、その接続先は `pnpm infra:up` が起動する `compose.yaml` の公開ポートに向きます。`runtime` container を起動する場合は `pnpm runtime:up` を使い、その container 内では `compose.yaml` から `.config/compose.toml` と `.config/compose.admin.toml` が渡されます。`setup.sh` は Docker daemon と Compose 構文だけを確認し、長時間生きる stack の起動までは行いません。
...
```bash
pnpm infra:up         # compose.yaml の shared infra（PostgreSQL / Valkey / OpenSearch / MinIO / Mailpit / SigNoz）を起動
pnpm infra:down       # compose.yaml の stack を停止
pnpm infra:ps         # compose.yaml の service 状態を確認
pnpm runtime:up       # optional な runtime container を profile 付きで起動
pnpm runtime:down     # runtime container を停止
```
...
- host / `devenv shell` の既定（`pnpm infra:up` が公開する port に接続）:
  - Product API: `.config/local.toml`
  - Admin API: `.config/local.admin.toml`
- Docker Compose runtime の既定（`pnpm runtime:up` 実行時のみ有効）:
...
| ファイル                   | 主要な接続先 / origin                                                                                                                                                                                   |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `.config/local.toml`       | PostgreSQL `localhost:5432`, Valkey `localhost:6379`, OpenSearch `http://localhost:9200`, MinIO `http://localhost:9000`, Mailpit SMTP `localhost:1025`, OTLP `127.0.0.1:4317`                           |
| `.config/local.admin.toml` | Admin origin `http://admin.localhost:5176`, Product origin `http://www.localhost:5173`, PostgreSQL `localhost:5432`, Valkey `localhost:6379`, OpenSearch `http://localhost:9200`, OTLP `127.0.0.1:4317` |
