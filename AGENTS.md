...
- When running verification from Codex Desktop or any host-side shell, invoke the required `pnpm` script from the Nix/devenv shell so the command uses the pinned toolchain instead of host Node.js, Go, bash, or pnpm. Example: `devenv shell -- pnpm check`.
